...
export default function EditForm({ open, handleOpen, handleClose, id }) {
  ...
  const fetchSingleGig = async () => {
    try {
      ...
      const { data } = await axios.get(url);
      setLoading(false);
      setGig((pre) => ({ ...pre, gigTitle: data.gig.title }));
      setGig((pre) => ({ ...pre, gigPrice: String(data.gig.price) }));
      setGig((pre) => ({ ...pre, description: data.gig.description }));
      setGig((pre) => ({ ...pre, Deliver: data.gig.deliveredTime }));
      setError("");
    } catch (error) {
      setLoading(false);
      if (
